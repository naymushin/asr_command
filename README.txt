Command ASR (arithmetic shift right) – shift of two-byte binary number
for 1 position to the right. Number is presented in two’s complement code
...
where DD – is destination, i.e. there are 3 bits for register number plus
...
I selected 000 as number of register, that is R0, though it’s possible
to use any of registers R0-R5 to write command in cells 0AF0h and 0AF1h.
Register R6 (SP – stack pointer) increases by 1 after execution the command
(in the task with indirect addressing). Register R7 (PC – program counter)
...
memory cells: 0B00h and 0B01h, i.e. in register R0 – in the task with direct
...
for commands for this processor looks as follows: XXXXXX, i.e. 6 octal characters.
But after conversion it to binary code we’ll have already 6 * 3 = 18 binary
characters, while registers are 16-bit. Have a look after machine codes for
assembler mnemonics, I’ve noticed all of them in highest octal position contain
